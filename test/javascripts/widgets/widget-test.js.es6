...
widgetTest("hbs template - no tagName", {
  ...
  beforeEach() {
    createWidget('hbs-test', {
      template: `<div class='test'>Hello {{attrs.name}}</div>`
    });
...
widgetTest('widget attaching', {
  ...
  beforeEach() {
    ...
    createWidget('attach-test', {
      ...
      template: `{{attach widget="test-embedded"}}`
    });
  },
